...
### ⏳ 目前是固定值 66，不是浮動帶寬（2026-08-26 起）

`femaleMin: 66 / femaleMax: 66` —— 畫面固定顯示 **女 66% / 男 34%**，永遠不動。
Morgan 2026-08-26 指示先寫死，**這是暫時值，不是最終數字**，他會依招募狀況再指示
要調成多少或何時切回浮動。66 沒有任何演算法上的依據，就是人工指定的一個數。

原理是 `policyFemalePct()` 開頭的 `if (hi <= lo) return Math.round(lo);` —— 兩端相等
就早退，完全不進遊走迴圈。**`join.js` 不用改**。實測 n=0~50000（含 `NaN`／負數／
`Infinity`／`1e9`）輸出恆為 66。

**要調整**：把 `femaleMin` 與 `femaleMax` 改成同一個新數字。
**要切回浮動**：改成兩個不同的值（例如 51 / 53），遊走邏輯自動恢復。

已知且已拍板的兩個取捨，不用再提報：

1. **文案已配合調整。** 性別比條正下方原本寫「樹屋會自動維持接近 1:1 的性別平衡」，
   與 66:34 直接打架。Morgan 2026-08-26 改成「**樹屋 App 正式推出後，會自動維持接近
   1:1 的性別平衡**」—— 把 1:1 的承諾移到未來式，與當下的 66:34 就不衝突了。
   （實測 360px 最窄視窗仍是 2 行、無溢出，CTA 首屏餘裕不變。）
2. **`data-status` 變成 `paused_female`。** 66% 超過 `pauseThreshold: 0.55`。
   目前全 repo 沒有任何 CSS/JS 讀這個屬性，畫面沒有表現、不會壞；csv 模式上線那天
   才需要處理（見「csv 模式上線前要補的三件事」第 1 點）。
...
訪客在同一頁往下滑：看板 **女66/男34** → 輪播 **男55/女45**，而且輪播那張是男性過半。
這是圖片，JS 蓋不掉、改文案也救不了。
...
1. **`data-status` 是 dead code。** `renderGenderRatio()` 會把 `open` /
   ...
   （帶寬 51–53 遠低於 `pauseThreshold: 0.55`）—— 但 2026-08-26 起的固定值 66
   已經超過門檻，`data-status` 現在就是 `paused_female`（畫面仍無表現）；csv 接上真實數字後
...
3. **四捨五入的誤差固定由男性吸收。** `renderGenderRatio()` 是女性先 `Math.round`、
   ...
   51–53 帶寬碰不到 .5（固定值 66 是整數，同樣碰不到），但 csv 的真實數字會。不想固定偏女性的話，改成「人數多的
...
## 男女比：49–53 → 51–53 → 當天稍晚改為固定 66

分兩步發生在同一天。**先**收窄帶寬 49–53 → 51–53（女性 51~53 ＝ 男性 47~49，
男性永遠不會過半），這是明確放棄 08-21 那個「帶寬跨過 50 才有真實感」的取捨。
**後**（同日 18:30）Morgan 指示直接寫死 **女 66%**，浮動整個停用 —— 見上面
「⏳ 目前是固定值 66」那節。51–53 的參數與說明都保留著，切回浮動時可直接用。詳細參數、實測數字、
...
3. `join/index.html` 的靜態預設值是 `width:45%` / `55%`、文字「女性 45% / 男性 55%」，
   **男性過半**，牴觸新規則。JS 一畫就蓋掉所以平常看不到，但 csv 模式第一次 fetch 前、
   以及不跑 JS 的文字擷取器讀到的就是它。已改成 66 / 34（對齊固定值）。同一個看板的靜態計數器也從 298 改為 742，與 baseValue 一致 —— 日後調整固定值或 baseValue，這兩處都要一起改。
