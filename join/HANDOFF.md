...
⚠️ 820 以後**永遠停在 58%**，不會繼續往下。這是階梯表的定義（超過最後一階就停住），
不是 bug。要繼續往下就加更多階。

⚠️ 797→800 那一階的間距是 3，其餘都是 10。照 Morgan 給的表原樣實作。

**時程實測推算**（速率 17.5 人/小時，由 758@20:41 → 796@22:51 推得）：
800 約 23:04、810 約 23:38、**820 約 00:13 鎖死**。
8/27 12:00 公測時約 1027 人，所以**公測當天整天都會是固定的 58%**，數字不會動。
Morgan 2026-08-26 22:51 知悉並選擇這樣（另一個選項是把間距拉開讓 58% 落在公測那一刻）。
...
| 2 | `join/index.html`（114-119 行） | 靜態預設 `width:` 與文字百分比，女男加總 100。目前設成**階梯終點 58/42** —— 因為 820 之後整個公測期都是這個值，設終點比設當下值更少過時 |
