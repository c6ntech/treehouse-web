...
`utm_content` 的 `dN` 是**招募起算第幾天**（d1 = 2026-08-20），不是「第幾篇貼文」。
沒發文的日子那個號碼就跳過，不補、不順延。

已知的空檔：**d9 ~ d13（8/28 ~ 9/1）沒有發文**，Morgan 2026-09-02 確認。
所以上表從 `0827d8` 直接跳到 `0902d14`，**這不是漏記**。

日後看到序號不連續，先對照這一節再判斷是不是漏了。
...
最後更新：2026-09-02（記錄 d9~d13 未發文的空檔）。規則有變請一起更新 `join/utm-tool/index.html`
