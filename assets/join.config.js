...
window.JOIN_CONFIG = {
  ...
  genderRatio: {
    ...
    // 49–53：女性 49~53，換算男性就是 47~51 —— 男性上限 51，不會再出現男 52、男 53。
    //        Morgan 2026-08-21 決定（原本 47–53，男性最多會走到 53，太多了）。
    // 仍然跨過 50 兩側：女 49 那格就是男性過半，只是變少（約 5% 的時間），
    // 留著是為了真實感 —— 從頭到尾女性都領先反而假。
    ...
    //   1.0    5%               5 ／ 5 人      ← 目前
    //
    // 上表是 49–53 帶寬下實測的（2026-08-21，n=130~1500）。帶寬縮到 49–53 之後
    // 男性過半只剩「女 49」這一格，所以連續長度固定就是 changeEverySignups，
    // 不會再像 47–53 那樣一次連續好幾十人；bias 只會改變它多久出現一次。
