...
window.JOIN_CONFIG = {
  ...
  genderRatio: {
    ...
    //            控制派發（目標 女55：男45）。
    //            ⚠️ 2026-08-26 起畫面顯示的是人工寫死的 66，**不是**那個派發政策，
    ...
    //
    // ⚠️ 固定 66 會踩到兩件事，切回浮動前請記得：
    //    1. 66% > pauseThreshold(0.55)，所以 data-status 會變成 "paused_female"
    //       （目前全 repo 沒有任何 CSS/JS 讀它，畫面沒有表現；csv 模式上線後才有影響）。
    //    2. 頁面上緊鄰的文案是「樹屋會自動維持接近 1:1 的性別平衡」，
    //       66:34 與它互相矛盾。Morgan 2026-08-26 知悉並決定文案不動，
    ...
    //    要重設帶寬的話，兩端都不要碰到 50。
    femaleMin: 66,      // ← 與 femaleMax 相等 = 固定值模式。浮動請用 51
    femaleMax: 66,      // ← 與 femaleMin 相等 = 固定值模式。浮動請用 53
